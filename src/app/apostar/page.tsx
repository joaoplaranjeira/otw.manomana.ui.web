...
export default function PredictionPage() {
  ...
  const [minimumPredictionDate] = useState(getTodayIso);
    ...
    <main className="flow-shell">
      ...
      <form className="flow-card page-enter" key={step} onSubmit={(event) => { event.preventDefault(); if (step === 6) submit(); else next(); }}>
        ...
        <div className="flow-actions">
          ...
          {step >= 2 && step <= 5 && <button className="skip" type="button" onClick={() => { if (step === 4) setWeight(""); if (step === 5) setHeight(""); next(); }}>Prefiro não adivinhar</button>}
